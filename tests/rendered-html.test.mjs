...
test("server-renders the complete audit AI course", async () => {
  ...
  assert.match(html, /审计应用功能全景：先选功能，不先选模型/);
  assert.match(html, /智能问数：从自然语言问题到可信数字/);
  assert.match(html, /智能生成审计报告：从已确认证据到可复核草稿/);
  assert.match(html, /共性底座、治理与试点上线/);
  ...
  assert.match(html, /模型不直接.*算出.*审计数字/s);
  assert.match(html, /运行一次受控智能问数/);
