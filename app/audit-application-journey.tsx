...
const auditRoute = [
  ...
  { id: "audit-collaboration", no: "18", title: "三个智能体协作系统", detail: "统一证据、权限、任务和数据接口" },
...
const collaborationSteps = [
  { name: "解析项目资料包", agent: "资料解析智能体" },
  { name: "提取合同、付款和供应商", agent: "资料解析智能体" },
  { name: "发现合同与台账金额差异", agent: "资料解析智能体" },
  { name: "查询历史采购数据", agent: "智能问数与分析智能体" },
  { name: "分析供应商三年趋势", agent: "智能问数与分析智能体" },
  { name: "识别单一来源比例异常", agent: "智能问数与分析智能体" },
  { name: "检索适用采购制度", agent: "报告生成智能体" },
  { name: "形成结构化审计发现", agent: "审计人员" },
  { name: "生成报告提纲", agent: "报告生成智能体" },
  { name: "生成问题与建议草稿", agent: "报告生成智能体" },
  { name: "展示证据和查询语句", agent: "报告生成智能体" },
  { name: "提交审计人员复核", agent: "审计人员" },
] as const;

export function AuditApplicationRoute({ onSelect }: { onSelect: (id: string) => void }) {
  return <nav className="audit-app-route" aria-label="第三部分章节目录">
    <header><span>第三部分 · 学习路线</span></header>
    <ol>{auditRoute.map(item => <li key={item.id}><button type="button" onClick={() => onSelect(item.id)}><b>{item.no}</b><strong>{item.title}</strong><span>{item.detail}</span></button></li>)}</ol>
  </nav>;
}
...
export function AuditDesignWorkbench({ initial = "documents" }: { initial?: DesignKey }) {
  ...
  const fields = [["目标是什么？", item.goal], ["输入是什么？", item.input], ["需要什么知识？", item.knowledge], ["可以调用什么工具？", item.tools], ["权限和边界是什么？", item.permission], ["输出怎样验证？", item.validation]];
  return <div className="audit-design-workbench"><div className="design-case-tabs">{(Object.keys(designCases) as DesignKey[]).map(key => <button type="button" key={key} className={active === key ? "active" : ""} onClick={() => setActive(key)}>{designCases[key].label}</button>)}</div><div className="design-six-grid" aria-live="polite">{fields.map(([question, answer]) => <article key={question}><strong>{question}</strong><p>{answer}</p></article>)}</div><blockquote>设计智能体的起点<strong className="key-term">不是选择模型</strong>，而是先明确目标、输入、知识、工具、权限和验证方式。</blockquote></div>;
}
...
export function AuditChainChallenge() {
  ...
  return <div className="audit-chain-challenge"><div className="audit-material-pack"><span>采购与合同管理专项审计 · 资料入口</span>{["合同扫描件", "合同附件", "现场照片", "电子表格台账", "文字处理制度", "会议纪要", "系统导出数据", "历史批准报告", "整改材料"].map(item => <b key={item}>{item}</b>)}</div><section><span>启发式问题</span><h3>把全部资料一次性交给大语言模型，能否直接完成审计？</h3><p>请选择系统的第一步。</p><div>{[["all", "全部内容直接交给模型"], ["route", "先识别类型、可读性与安全状态"], ["convert", "把所有文件统一转换成图片"]].map(([key, label]) => <button type="button" key={key} className={choice === key ? "active" : ""} onClick={() => setChoice(key as "all" | "route" | "convert")}>{label}</button>)}</div><blockquote className={choice === "route" ? "ok" : ""}>{feedback}</blockquote><div className="three-agent-answer"><article><b>资料解析智能体</b><p>资料怎样进入系统并成为<strong className="key-term">可靠证据</strong>？</p></article><article><b>智能问数与分析智能体</b><p>怎样安全、准确地找到数字并分析？</p></article><article><b>报告生成智能体</b><p>怎样把<strong className="key-term">事实、证据和分析</strong>组织成报告？</p></article></div></section></div>;
}
...
GROUP BY company_name;`;
  return <div className="secure-query-lab"><header><div><span>教学模拟 · 不连接真实数据库</span><h3>2025年上半年，各单位单一来源采购金额和同比情况</h3></div><div className="role-tabs">{(Object.keys(roleResults) as RoleKey[]).map(key => <button type="button" key={key} className={role === key ? "active" : ""} onClick={() => setRole(key)}>{roleResults[key].label}</button>)}</div></header><div className="query-stepper" aria-label="安全问数步骤">{querySteps.map((label, index) => (
    <Fragment key={label}>
      <button type="button" className={index === step ? "active" : index < step ? "done" : ""} onClick={() => setStep(index)}><b>{String(index + 1).padStart(2, "0")}</b><span>{label}</span></button>
      {index < querySteps.length - 1 && <i aria-hidden="true">→</i>}
    </Fragment>
  ))}</div><section className="query-workbench"><div><span>业务语义与查询计划</span><dl><dt>指标</dt><dd>单一来源采购金额</dd><dt>定义</dt><dd>采购方式为“单一来源”的合同金额合计</dd><dt>时间</dt><dd>合同签订日期 · 2025年上半年</dd><dt>分组</dt><dd>二级单位；同比；供应商下钻</dd><dt>权限</dt><dd>{item.scope}</dd></dl><small>数据库字段说明数据在哪里；语义层说明业务指标是什么意思。</small></div><div><span>结构化查询语句草稿</span><pre>{query}</pre><p><b>:授权单位范围</b>由<strong className="key-term">权限服务</strong>提供，模型<strong className="key-term">不得自行填写</strong>。</p></div><div><span>执行前决策</span><div className="query-decisions">{[["run", "直接执行"], ["validate", "校验并注入权限"], ["select", "只要只读就执行"]].map(([key, label]) => <button type="button" key={key} className={decision === key ? "active" : ""} onClick={() => setDecision(key as "run" | "validate" | "select")}>{label}</button>)}</div><blockquote className={decision === "validate" ? "ok" : ""}>{decisionText}</blockquote><ul>{["仅允许只读单语句", "表字段白名单", "组织/项目/行/字段权限", "扫描行数与超时限制", "敏感字段脱敏", "异常关联与危险函数检查", "完整查询日志"].map(rule => <li key={rule}>{rule}</li>)}</ul></div></section><div className="query-results" aria-live="polite"><div><span>当前身份</span><strong>{item.label}</strong><p>{item.scope}</p></div><table><thead><tr><th>单位</th><th>金额</th><th>同比</th></tr></thead><tbody>{item.rows.map(row => <tr key={row[0]}>{row.map(cell => <td key={cell}>{cell}</td>)}</tr>)}</tbody></table><div><span>自动分析</span><strong>{role === "headquarters" ? "天津分公司同比增长最高，为32%。" : `${item.rows[0][0]}在当前授权范围内金额最高。`}</strong><p>总部视图进一步拆分发现：某技术服务公司贡献新增金额620万元，占天津新增金额57%。建议检查<strong className="key-term">连续单一来源采购、合同拆分和采购理由</strong>。</p></div></div><footer><span>可追溯记录</span><b>原始问题</b><b>指标口径</b><b>权限范围</b><b>数据表</b><b>执行语句</b><b>查询时间</b><b>数据更新时间</b><b>返回记录数</b><b>脱敏状态</b></footer></div>;
}
...
export function ReportGenerationStudio() {
  ...
  return <div className="report-generation-studio"><header><div><span>报告生成准备向导</span><h3>先确认类型、范围、发现、证据和风格，再生成正文</h3></div><div>{["专项审计报告", "审计问题段落", "审计调查报告", "整改报告"].map(item => <button type="button" key={item} className={type === item ? "active" : ""} onClick={() => setType(item)}>{item}</button>)}</div></header><div className="report-knowledge-layers">{knowledgeLayers.map(([name, detail]) => <article key={name}><strong>{name}</strong><p>{detail}</p></article>)}</div><section className="report-preparation"><div><span>任务范围</span><dl><dt>报告类型</dt><dd>{type}</dd><dt>审计对象</dt><dd>某分公司</dd><dt>审计期间</dt><dd>2023年1月至2025年12月</dd><dt>审计领域</dt><dd>数据治理项目管理</dd><dt>证据状态</dt><dd>两个发现已确认；一项范围待核实</dd></dl></div><div><span>结构化审计发现</span><strong>发现-001 · 部分历史数据存在重复治理</strong><p><strong className="key-term">事实绑定证据</strong>-101、证据-102、证据-201；<strong className="key-term">制度条款绑定规则</strong>-001。</p><p><b>不确定性：</b><strong className="key-term">部分项目实际治理对象仍需进一步核实</strong>。</p></div><div><span>可编辑风格规则</span><div className="style-tabs">{["正式、客观、克制", "简洁、数字优先", "问题与建议并列"].map(item => <button type="button" key={item} className={style === item ? "active" : ""} onClick={() => setStyle(item)}>{item}</button>)}</div><p>重要结论后紧跟金额、时间和来源；<strong className="key-term">证据不足写“尚需进一步核实”</strong>；禁止使用“显而易见”“完全证明”。</p></div></section><div className="outline-editor"><span>先确认提纲，再分章节生成</span>{outline.map((item, index) => <div key={item}><b>{index + 1}</b><strong>{item}</strong><button type="button" aria-label={`上移${item}`} disabled={index === 0} onClick={() => move(index, -1)}>↑</button><button type="button" aria-label={`下移${item}`} disabled={index === outline.length - 1} onClick={() => move(index, 1)}>↓</button></div>)}</div><section className="report-review-workbench"><aside><span>章节与问题</span>{outline.map((item, index) => <button type="button" key={item} className={item === "主要问题" ? "active" : ""}>{index + 1}. {item}</button>)}</aside><main><span>生成正文 · {style}</span><h4>（一）部分历史数据存在重复治理</h4><p>审计发现，部分历史数据存在重复治理情况。2017年12月至2020年6月，相关单位开展历史数据补建、清理、结构化采集和入湖归档；2021年底至2022年底，又面向相近范围开展数据清洗迁移。</p><p>上述项目在治理对象和工作内容方面<strong className="key-term">存在一定重叠</strong>，一定程度上反映出<strong className="key-term">项目统筹和范围审核机制</strong>不够健全。</p><div className={`review-state ${review}`}><strong>{review === "accepted" ? "段落已由审计人员接受并锁定" : review === "returned" ? "已退回：请保留不确定性并补充范围证据" : "等待审计人员审核"}</strong></div><div><button type="button" onClick={() => setReview("accepted")}>接受并锁定</button><button type="button" onClick={() => setReview("returned")}>退回修改</button><button type="button" onClick={() => setReview("pending")}>恢复待审</button></div></main><aside><span>证据与质量检查</span><b>证据-101：立项文件第3页</b><b>证据-102：验收报告第7页</b><b>证据-201：项目任务书第5页</b><b>规则-001：制度第三章第十二条</b>{["主体名称一致", "日期范围一致", "关键事实均有来源", "制度版本有效", "不确定性已保留", "模板与风格通过"].map(item => <small key={item}>✓ {item}</small>)}</aside></section></div>;
}

export function ReportArchitectureBoard() {
  return <div className="report-architecture-board"><div><span>第一层</span><strong>事实与证据</strong><p>确认发生了什么、数字是多少、证据在哪里；不足则<strong className="key-term">阻断确定性结论</strong>。</p></div><i>↓</i><div><span>第二层</span><strong>审计分析</strong><p>事实 → 有效标准 → 差异 → 原因 → 影响 → 建议；<strong className="key-term">制度不得由模型创造</strong>。</p></div><i>↓</i><div><span>第三层</span><strong>报告表达</strong><p>按类型、模板和已确认风格组织文字；<strong className="key-term">不得修改事实与数字</strong>。</p></div><blockquote>初期优先：结构化模板 + 发现数据模型 + 证据绑定 + 检索增强生成 + 风格配置 + 分章节生成 + 规则校验 + 人工审核。<strong className="key-term">微调不能替代证据检索和事实校验</strong>。</blockquote></div>;
}

export function AuditCollaborationLab() {
  const [active, setActive] = useState(0);
  const item = collaborationSteps[active];
  return <div className="audit-collaboration-lab">
    <div className="collaboration-timeline" aria-label="协作步骤时间线">
      {collaborationSteps.map((step, index) => (
        <Fragment key={step.name}>
          ...
          {index < collaborationSteps.length - 1 && <i aria-hidden="true">→</i>}
        </Fragment>
      ))}
    </div>
    <section aria-live="polite">
      <span>完整任务 · 步骤 {active + 1}/{collaborationSteps.length}</span>
      <h3>{item.name}</h3>
      <div className="collaboration-owner">
        <span>当前责任</span>
        <strong>{item.agent}</strong>
        <p>每一步都继承同一项目身份、权限范围、证据编号和运行日志。</p>
      </div>
      <button type="button" onClick={() => setActive((active + 1) % collaborationSteps.length)}>
        {active === collaborationSteps.length - 1 ? "重新运行" : "执行下一步 →"}
      </button>
    </section>
    <div className="shared-foundation"><span>三个智能体共享</span>{["用户身份", "组织与项目权限", "文件与数据权限", "统一证据编号", "审计知识库", "工具注册中心", "任务状态", "操作日志", "人工审批", "安全控制", "模型调用服务", "版本管理"].map(item => <b key={item}>{item}</b>)}</div>
    <div className="audit-object-interfaces">{[["证据对象", "来源类型 · 原始位置 · 内容 · 置信度 · 权限"], ["查询结果对象", "问题 · 指标口径 · 查询语句 · 数据时间 · 结果 · 分析"], ["审计发现对象", "事实 · 标准 · 原因 · 影响 · 建议 · 证据 · 复核状态"], ["报告对象", "类型 · 范围 · 提纲 · 章节 · 发现 · 模板 · 风格 · 审核状态"]].map(([name, detail]) => <div key={name}><strong>{name}</strong><p>{detail}</p></div>)}</div>
  </div>;
}
