...
export function AuditDesignWorkbench({ initial = "documents" }: { initial?: DesignKey }) {
  ...
  return <div className="audit-design-workbench"><div className="design-case-tabs">{(Object.keys(designCases) as DesignKey[]).map(key => <button type="button" key={key} className={active === key ? "active" : ""} onClick={() => setActive(key)}>{designCases[key].label}</button>)}</div><div className="design-six-grid" aria-live="polite">{fields.map(([question, answer]) => <article key={question}><strong>{question}</strong><p>{answer}</p></article>)}</div><blockquote>设计智能体的起点不是选择模型，而是先明确目标、输入、知识、工具、权限和验证方式。</blockquote></div>;
}
