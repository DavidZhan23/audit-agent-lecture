...
export function AgentChapterRoute() {
  return <ChapterRoute label="第二部分 · 学习路线" title="定义 → 运行 → 控制 → 建设" items={[
    ["06", "什么是智能体", "从一次回答到行动—反馈闭环"],
    ["07", "智能体怎样运行", "模块、工具、状态与反馈循环"],
    ["08", "智能体怎样受控", "风险、自主度与控制生命周期"],
    ["09", "智能体怎样落地", "价值、评价与建设顺序"],
  ]} />;
}
...
export function AgentFitLab() {
  ...
  const tasks: Record<FitKey, { name: string; facts: string[]; verdict: string; reason: string; build: string }> = {
    calculator: { name: "差旅标准计算器", facts: ["公式明确", "路径固定", "不需要外部反馈"], verdict: "普通程序", reason: "输入与答案关系可以完整写出，用智能体只会增加成本和不确定性。", build: "函数 + 规则测试" },
    workflow: { name: "新员工入职办理", facts: ["步骤预先定义", "分支有限", "多系统串联"], verdict: "固定工作流", reason: "虽然连接多个系统，但下一步主要由事先设计的流程图决定。", build: "流程引擎 + 调用接口" },
    assistant: { name: "根据材料起草摘要", facts: ["一次输入输出", "不主动取数", "结果由人使用"], verdict: "大语言模型应用", reason: "模型只需理解已提供材料并生成文本，没有持续行动与反馈闭环。", build: "检索/提示 + 大语言模型" },
    investigation: { name: "跨系统调查异常订单", facts: ["路径事前不完整", "查询结果改变下一步", "需要状态与停止条件"], verdict: "适合智能体", reason: "任务需要围绕目标动态选择工具、读取结果并决定继续、改道或停止。", build: "大语言模型决策 + 工具 + 状态 + 编排 + 控制" },
  };
...
export function AgentArchitectureExplorer() {
  ...
  const modules = [
    ...
    { name: "大语言模型决策核", question: "基于当前状态，下一步是什么？", input: "目标、证据、工具说明、历史步骤", output: "回答、计划或结构化工具调用", risk: "可能选错工具、编造参数或过度自信" },
    ...
    { name: "编排循环", question: "怎样让观察真正影响下一步？", input: "模型行动 + 工具观察", output: "更新状态、继续、停止或升级", risk: "固定循环不是动态智能体；无限循环会失控" },
...
export function ToolContractLab() {
  ...
  const results = {
    ok: { label: "查询成功", status: "ok", payload: "arrival_city: 南京\nsource: flight_records.csv / T2017", next: "城市与报销说明矛盾 → 继续查询酒店与客户关系管理系统" },
...
export function AgentControlLab() {
  ...
  const levels = [
    ...
    { name: "只读调查", desc: "允许在白名单内自动读取", allowed: ["查航班/酒店/客户关系管理系统", "计算与比对", "形成证据包"], approval: ["扩大数据范围"], denied: ["修改源数据", "自动定性"] },
...
export function AuditCapabilityMap() {
  ...
  const items: Record<CapabilityKey, { name: string; question: string; input: string; output: string; stack: string; boundary: string }> = {
    ask: { name: "智能问数", question: "华东区二季度差旅费同比增长多少？", input: "自然语言问题 + 数据目录 + 指标口径", output: "数字、口径、查询、来源和限制", stack: "语义层 + 结构化查询语言工具 + 大语言模型解释 + 校验", boundary: "答案只代表获准数据与确定口径，不能自动成为审计结论。" },
    screen: { name: "智能风险筛查", question: "哪些报销最值得优先核查？", input: "规则、特征、历史标签和当期待审数据", output: "疑点队列、触发规则和风险信号", stack: "规则 + 机器学习/神经网络 + 阈值策略", boundary: "概率用于排序，不能代替证据。" },
    investigate: { name: "智能取证调查", question: "该行程矛盾还缺哪些证据？", input: "明确目标 + 只读工具 + 当前证据状态", output: "来源可追溯的证据包和未决事项", stack: "智能体 + 工具 + 状态 + 停止条件", boundary: "只能在批准范围内取数；访谈和扩围需要人工关口。" },
    workpaper: { name: "智能底稿助手", question: "怎样把证据整理成标准工作底稿？", input: "已复核证据、程序模板和审计记录", output: "带引用的底稿草稿和复核清单", stack: "模板引擎 + 大语言模型 + 引用校验", boundary: "机器草稿必须标记并保留人工修订链。" },
    report: { name: "智能报告生成", question: "怎样从已确认发现形成报告草稿？", input: "冻结发现库、证据、回应和报告模板", output: "带来源的章节草稿、图表和版本记录", stack: "发现数据模型 + 大语言模型 + 确定性校验 + 审批流", boundary: "不得补写原因、影响、评级或管理层回应。" },
...
export function AskDataLab() {
  ...
  const cases: Record<AskDataMode, { label: string; question: string; status: string; state: "ok" | "clarify" | "denied"; contract: string[]; plan: string; query: string; answer: string; proof: string[] }> = {
    ...
    denied: { label: "权限拒绝", question: "列出全部员工身份证号、工资和差旅明细。", status: "拒绝执行并留下日志", state: "denied", contract: ["请求包含高敏个人信息", "当前角色只有费用汇总权限", "目的与最小必要原则不匹配", "不存在已批准的数据扩围"], plan: "权限引擎在生成结构化查询语言前阻断；不得通过别名、分批查询或其他工具绕过。", query: "策略阻断：工资数据读取权限未授权", answer: "无法执行该查询。可以提供经脱敏、按部门汇总的差旅指标；如确有审计需要，请走数据扩围审批。", proof: ["策略：最小数据原则第七条", "所需权限：工资数据读取", "当前权限：费用汇总读取", "事件已写入安全日志"] },
  };
...
export function AuditScenarioSelector() {
  ...
  const scenes: Record<SceneKey, { name: string; verdict: string; scores: number[]; reason: string; route: string }> = {
    travel: { name: "差旅报销初审", verdict: "适合作为首个试点", scores: [5,5,4,5,5], reason: "高频、数据较齐、证据字段明确，并且天然有审计人员复核。", route: "规则筛查 + 模型理解 + 智能体只读调查" },
    duplicate: { name: "重复发票检查", verdict: "值得做，但不必先用智能体", scores: [5,5,5,5,5], reason: "条件和路径高度确定，普通代码/工作流更便宜、更稳定、更易解释。", route: "优先规则与固定工作流" },
    opinion: { name: "自动签发审计意见", verdict: "不应自动化", scores: [1,2,1,1,1], reason: "涉及重大职业判断、责任承担和正式对外结论，不能交给智能体自主完成。", route: "只允许辅助整理，结论由人" },
...
export function AuditEvidenceMap() {
  const sources = ["报销", "发票", "审批", "航班", "酒店", "客户关系管理系统", "光学字符识别", "日历", "员工主数据"];
  return <div className="audit-evidence-map"><div className="evidence-sources"><span>事实来源 · 9张表</span><div>{sources.map(source => <b key={source}>{source}</b>)}</div></div><i>通过报销编号、发票编号、行程编号和员工编号关联</i><div className="evidence-policy"><span>适用标准 · 2份制度</span><strong>费用制度</strong><strong>特殊事项通知</strong><p>必须保存版本、生效日期和检索段落。</p></div><i>进入受控处理层</i><div className="evidence-processing"><span>能力组合</span><div><b>规则</b><b>机器学习</b><b>神经网络</b><b>大语言模型</b><b>智能体</b></div><p>每个结果都携带来源、时间、关联键和处理记录。</p></div><i>形成</i><div className="evidence-output"><span>审计交付</span><strong>可复核疑点证据包</strong><p>不是单独一个风险分数，也不是模型自动定性。</p></div></div>;
}
...
export function EvidencePackageLab() {
  ...
  const parts = [
    ...
    { name: "证据 Evidence", value: "航班、酒店、客户关系管理系统、日历四个来源共同指向南京，无苏州客户拜访。", source: "T2017 / E1004 的关联记录及查询时间", check: "证据要可回到原始记录重新核对" },
    { name: "不确定性 Uncertainty", value: "尚未取得员工解释；不能排除临时改签、代办或记录缺失。", source: "工具均成功，但人员访谈未完成", check: "明确缺口，不把“未发现”写成“不存在”" },
    { name: "建议 Action", value: "转人工复核；取得行程变更说明、客户证明和审批附件。", source: "智能体停止原因：证据矛盾达到升级阈值", check: "建议下一步，不自动认定违规或舞弊" },
  ];
...
export function AuditResponsibilityLab() {
  ...
  const steps = [
    ...
    { name: "跨系统取证", system: "智能体在白名单内查询航班、酒店、客户关系管理系统和制度", human: "批准扩围、高敏数据和任何写操作", handoff: "原始返回 + 参数 + 时间 + 失败记录", reason: "动态查询适合智能体，但权限和范围必须由人预设。" },
